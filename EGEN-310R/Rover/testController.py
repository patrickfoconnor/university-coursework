...
if __name__ == '__main__':
    ...
    def leftThumbX(xValue):
        ...
        if xValue < 0:
            #print"Turning Left"
            rover.turnLeft(xValue)
        elif xValue > 0:
            rover.turnRight(xValue)
            #print"Turning Right"
        else:
            rover.centerSteering()
            print"Centered steering"
    ...
    def buttonA(value):
        if value == 1:
            ...
            print("lights off")
            #rover.winchNeutral()
        #print "A Button Value = {}".format(value)


    def buttonB(value):
        #print "B Button Value = {}".format(value)
        if value == 1:
            #print("winch forward")
            rover.winchForward()
        else:
    ...
    def buttonX(value):
        print "X Button Value = {}".format(value)


    def buttonY(value):
        #print "Y Button Value = {}".format(value)
        if value == 1:
            #print("winch backwards")
            rover.winchReverse()
        else:
            #print("winch neutral")
            rover.winchNeutral()
    ...
    # -----------Bumper calls for winch forward and reverse---------------------------------------------------
    # def rightBumper(value):
    #     # print "Left Bumper Value = {}".format(value)
    #     if value == 1:
    #         print("winch forward")
    #         # rover.winchForward()
    #     else:
    ...
    #
    #
    def leftBumper(value):
        # print "Right Bumper Value = {}".format(value)
        if value == 1:
            #print("Rock Crawl")
            rover.rockCrawl()
        else:
    ...
    def rightTrigger(value):
        # print "Right Bumper Value = {}".format(value)
        if value > 0:
            #print("drive forwards: ", value)
            rover.driveForward(value)
        else:
            #print("drive neutral")
            rover.driveNeutral()
    ...
    def leftTrigger(value):
        # print "Right Bumper Value = {}".format(value)
        if value > 0:
            #print("drive backwards: ", value)
            rover.driveBackward(value)
        else:
            #print("drive neutral")
            rover.driveNeutral()
    def cancelProgram(value):
        print("cancelProgram")
        rover.usb.setTarget(0x00, 6000)
        rover.usb.setTarget(0x01, 6000)
        rover.usb.setTarget(0x02, 6000)
        #NEED TO DO rover.led(OFF)
        testController.stop()


    # setup xbox controller, set out the deadzone and scale, also invert the Y Axis (for some reason in Pygame
    # negative is up - wierd!
    xboxCont = testController(controlCallBack, deadzone=30, scale=100, invertYAxis=True)

    # setup the left and right thumb (X & Y) callbacks
    ...
    # setup the a,x,y,b  callbacks
    xboxCont.setupControlCallback(xboxCont.XboxControls.A, buttonA)
    xboxCont.setupControlCallback(xboxCont.XboxControls.B, buttonB)
    xboxCont.setupControlCallback(xboxCont.XboxControls.X, buttonX)
    xboxCont.setupControlCallback(xboxCont.XboxControls.Y, buttonY)
    # setup the right and left bumper  callbacks
    xboxCont.setupControlCallback(xboxCont.XboxControls.LB, leftBumper)
    # xboxCont.setupControlCallback(xboxCont.XboxControls.RB, rightBumper)
    # setup the callbacks for right and left trigger for throttle
    ...
    # Get d-pad as the cancel program button
    # when canceled set all servos, motors, and lights off
    xboxCont.setupControlCallback(xboxCont.XboxControls.DPAD, cancelProgram)
