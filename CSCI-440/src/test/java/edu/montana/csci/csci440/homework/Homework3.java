...
public class Homework3 extends DBTest {
    ...
    public void selectPopularTracksAndTheirAlbums() throws SQLException {
        ...
        List<Map<String, Object>> tracks = executeSQL("SELECT tracks.Name as TrackName, " +
                "COUNT(invoice_items.TrackId) as TracksSales, tracks.AlbumId as Album\n" +
                "FROM tracks\n" +
        ...
        List<Map<String, Object>> albums = executeSQL("SELECT DISTINCT(albums.Title) as AlbumName,\n" +
                "COUNT(ii.TrackId) as singleTracksSales\n" +
                "FROM albums\n" +
                "    JOIN tracks t on albums.AlbumId = t.AlbumId\n" +
                "    JOIN invoice_items ii on t.TrackId = ii.TrackId\n" +
                "    GROUP BY ii.TrackId\n" +
                "        HAVING singleTracksSales > 1;");
     ...
     *
     * Please use an IN clause and a sub-select to generate customer
     * IDs satisfying the criteria
     * */
    public void selectCustomersMeetingCriteria() throws SQLException {
        // HINT: join to invoice items and do a group
        // by/having to get the right answer
        List<Map<String, Object>> tracks = executeSQL("SELECT customers.Email as CustomerEmails, customers.CustomerId\n" +
                "FROM customers\n" +
                "JOIN employees e on e.EmployeeId = customers.SupportRepId\n" +
                "WHERE e.FirstName LIKE \"Jane\" IN\n" +
